...
namespace Jango.TCPClient
{
    public class ClientSocket
    {
        ...
        public static SocketError Connect()
        {
            ...
            //我这里是读取配置,   
            //string ip = Config.Get<String>("socket_server");   //( ("socket", "server", "");
            //int port = Config.Get<Int32>("socket_port");// "socket", "port", 13909);
            //if (string.IsNullOrWhiteSpace(ip) || port <= 1000) return SocketError.Fault;
            var remoteServer = Jango.Common.NetWork.Sockets.SocketUtils.GetLocalIPV4();
            //创建连接对象, 连接到服务器  
            smanager = new SocketManager(remoteServer, 5000);
        ...
        private static void StartHeartbeat()
        {
            ...
            heartRes.data.Add("beat", Function.Base64Encode("now is " + DateTime.Now.ToString("HH:mm:ss")));
        }
    ...
    {
        private readonly ApiCode _apiCode;
